...
struct Compiler {
    ...
    method: Option<TypedHandle<Method>>,
...
impl Compiler {
    ...
    fn add_method(&mut self, function_type: FunctionType, name: TypedHandle<String>) {
        self.function_type = function_type;
        self.class.methods.push(Method::new(name))
    }
    ...
    fn method(&mut self) -> &mut Method {
        let index = self.class.methods.len() - 1;
        &mut self.class.methods[index]
    }
    fn count(&mut self) -> usize {
        self.method().chunk.count()
    }
    fn chunk(&mut self) -> &mut Chunk {
        &mut self.method().chunk
    }

    fn make_constant(&mut self, value: Value) -> Result<u8, String> {
        let constants = &mut self.class.constants;
...
impl Parser<'_> {
    ...
    fn emit_bytes(&mut self, bytes: &[u8]) {
        ...
        self.current_compiler.chunk().write(bytes, line)
    }
    ...
    fn dot(&mut self, can_assign: bool) -> Result<(), String> {
        let index = self.consume_member("Expect property name after '.'.")?;
    ...
    fn consume_member(&mut self, error_msg: &str) -> Result<u8, String> {
        self.consume(TokenType::Identifier, error_msg)?;
        let name = self.previous_token.lexeme(self.source);
    ...
    fn literal(&mut self, token_type: &TokenType) {
        self.emit_bytes(&[match token_type {
            ...
            _ => panic!(
                "'{}' mistaken for literal",
                self.previous_token.lexeme(self.source)
            ),
        }]);
    }
    ...
    fn number(&mut self) -> Result<(), String> {
        match self.previous_token.lexeme(self.source).parse::<f64>() {
    ...
    fn string(&mut self) -> Result<(), String> {
        let lexeme = self.previous_token.lexeme(self.source);
    ...
    fn super_(&mut self) -> Result<(), String> {
        ...
        let index = self.consume_member("Expect superclass method name.")?;
    ...
    fn parse_variable(&mut self, error_msg: &str) -> Result<u8, String> {
        let name = self.current_token.lexeme(self.source);
        self.consume(TokenType::Identifier, error_msg)?;
    ...
    fn method(
        ...
        let string = self.previous_token.lexeme(self.source).to_string();
        let name = self.heap.store(string);
        self.current_compiler.add_method(function_type, name);
        ...
        if !self.check(TokenType::RightParen) {
            ...
            self.current_compiler.method().arity = arity;
        }
        ...
        // that is it.
        // no ending the compiler,
        // no pushing methods around,
        // no no no!
        Ok(())
    }
